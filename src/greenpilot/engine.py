"""Orchestration: load sample AWS data, run every rule set, assemble a Report.
...
from .models import Finding, Report, Resource
...
def run_governance_rules(resources, carbon) -> list[Finding]:
    findings: list[Finding] = []
...
def analyze(data_dir: Path, company_name: str = "Sample Company") -> Report:
    ...
    governance_findings = run_governance_rules(resources, carbon)
